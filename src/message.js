...
var msg = {
  ...
  portAlreadyUsed: function (port) {
    err(port + ' is already used by other service, try other numbers');
  },
  apiAlreadyRunning: function (port) {
    err('server is already running in port ' + port);
  },
  ...
  requestProxied: function(path){
    success('request proxied ' + path);
  },
