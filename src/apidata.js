...
function prepareTemplates(templates){
    var valueGeneratorsDict = {
        ...
        unique_num: function(obj){
            ...
            if(!types[obj.type]){
                return log.noTemplateType('unique_num', obj.type, Object.keys(types))
            }
            if(obj.type == 'random' && !(obj.max_num&&obj.min_num)){
                return log.missingSettings('unique_num', 'random', ['max_num','min_num'])
            }
            if(obj.type == 'incremental'){
                if(!(obj.starting_num&&obj.increments)){
                    return log.missingSettings('unique_num', 'incremental', ['starting_num','increments'])
                }
        ...
        datetime: function(obj){
            ...
            if(obj.type == 'incremental'){
                if(!(obj.starting_datetime&&obj.increments&&obj.output_format)){
                    return log.missingSettings('unique_num', 'incremental', ['starting_datetime','increments','output_format'])
                }
